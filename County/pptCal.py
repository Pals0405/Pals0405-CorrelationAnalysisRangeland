...
for eco in prod1:
    ...
    # subset of tavg
    tavgL = climateProd[['year','Week_Number','PPT','productivity']]
    for y in range(1984,2022):
        tavg = tavgL[tavgL['year']==y]
        for i in range(1,27,2):
            tavg = tavg.reset_index(drop=True)
            rolling = tavg['PPT'].rolling(window=i,center=True).mean().fillna(-1).reset_index()
            rolling = rolling.rename(columns={'PPT': 'PPT'+str(-i)})
            roll = rolling['PPT'+str(-i)]
            tavg = tavg.join(roll)
    ...
    for w in list(tavgAll.Week_Number.unique()):
        ...
        ans = []
        ans.append(w)
        for column in tWeek.loc[:,~tWeek.columns.isin(['year','Week_Number','PPT','productivity'])]:
    ...
    tcorr.to_csv(path+'/PPTcorr.csv')
# figure, axis = plt.subplots(2)

# axis[0].plot(tcorr['Week_Number'],tcorr['p_value'])
# axis[0].set(xlabel='PPT', ylabel='-logP')
...
# axis[1].set(xlabel='PPT', ylabel='r')
