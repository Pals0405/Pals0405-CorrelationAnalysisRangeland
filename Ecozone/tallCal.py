...
for eco in prod1:
    ...
    # subset of tavg
    tavgL = climateProd[['year','Week_Number','Ref_ET_Tallgrass','productivity']]
    for y in range(1984,2022):
        tavg = tavgL[tavgL['year']==y]
        for i in range(1,27,2):
            tavg = tavg.reset_index(drop=True)
            rolling = tavg['Ref_ET_Tallgrass'].rolling(window=i,center=True).mean().fillna(-1).reset_index()
            rolling = rolling.rename(columns={'Ref_ET_Tallgrass': 'Ref_ET_Tallgrass'+str(-i)})
            roll = rolling['Ref_ET_Tallgrass'+str(-i)]
            tavg = tavg.join(roll)
    ...
    for w in list(tavgAll.Week_Number.unique()):
        ...
        ans = []
        ans.append(w)
        for column in tWeek.loc[:,~tWeek.columns.isin(['year','Week_Number','Ref_ET_Tallgrass','productivity'])]:
            print(tWeek['productivity'])
            print(tWeek[column])
    ...
    tcorr.to_csv(path+'/Ref_ET_Tallgrasscorr.csv')

# figure, axis = plt.subplots(2)

# axis[0].plot(tcorr['Week_Number'],tcorr['p_value'])
# axis[0].set(xlabel='Ref_ET_Tallgrass', ylabel='-logP')
...
# axis[1].set(xlabel='Ref_ET_Tallgrass', ylabel='r')
