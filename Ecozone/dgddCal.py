...
for eco in prod1:
    ...
    # subset of tavg
    tavgL = climateProd[['year','Week_Number','dayGDD','productivity']]
    for y in range(1984,2022):
        tavg = tavgL[tavgL['year']==y]
        for i in range(1,27,2):
            tavg = tavg.reset_index(drop=True)
            rolling = tavg['dayGDD'].rolling(window=i,center=True).mean().fillna(-1).reset_index()
            rolling = rolling.rename(columns={'dayGDD': 'dayGDD'+str(-i)})
            roll = rolling['dayGDD'+str(-i)]
            tavg = tavg.join(roll)
    ...
    for w in list(tavgAll.Week_Number.unique()):
        ...
        ans = []
        ans.append(w)
        for column in tWeek.loc[:,~tWeek.columns.isin(['year','Week_Number','dayGDD','productivity'])]:
    ...
    tcorr.to_csv(path+'/dayGDDcorr.csv')
# figure, axis = plt.subplots(2)

# axis[0].plot(tcorr['Week_Number'],tcorr['p_value'])
# axis[0].set(xlabel='dayGDD', ylabel='-logP')
...
# axis[1].set(xlabel='dayGDD', ylabel='r')
